...
import { LocationService } from './location-table.service';

@Injectable()
export class LocationEffects extends TableEffects {
	public constructor(
		actions$: Actions,
	...
	public getLocation$ = createEffect(() =>
		this.actions$.pipe(
			ofType(LocationTableActions.GetSelectedLocationPending),
			mergeMap(({ id }) =>
				this._service.getLocation(id).pipe(
					map((selectedLocation: any) => {
						return LocationTableActions.GetSelectedLocationSuccess({ selectedLocation });
					}),
					catchError(() => of(LocationTableActions.GetSelectedLocationError())),
				),
			),
		),
	);
