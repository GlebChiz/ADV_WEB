import { InjectionToken } from '@angular/core';

export const GET_TABLE_DATA_PENDING: InjectionToken<string> = new InjectionToken(
	'GET_TABLE_DATA_PENDING',
);

export const GET_TABLE_DATA_ERROR: InjectionToken<string> = new InjectionToken(
	'GET_TABLE_DATA_ERROR',
);

export const GET_TABLE_DATA_SUCCESS: InjectionToken<string> = new InjectionToken(
	'GET_TABLE_DATA_SUCCESS',
);

export const DELETE_ITEM_TABLE_PENDING: InjectionToken<string> = new InjectionToken(
	'DELETE_ITEM_TABLE_PENDING',
);

export const DELETE_ITEM_TABLE_ERROR: InjectionToken<string> = new InjectionToken(
	'DELETE_ITEM_TABLE_ERROR',
);

export const DELETE_ITEM_TABLE_SUCCESS: InjectionToken<string> = new InjectionToken(
	'DELETE_ITEM_TABLE_SUCCESS',
);

export const DUBLICATE_ITEM_TABLE_PENDING: InjectionToken<string> = new InjectionToken(
	'DUBLICATE_ITEM_TABLE_PENDING',
);

export const DUBLICATE_ITEM_TABLE_ERROR: InjectionToken<string> = new InjectionToken(
	'DUBLICATE_ITEM_TABLE_ERROR',
);

export const DUBLICATE_ITEM_TABLE_SUCCESS: InjectionToken<string> = new InjectionToken(
	'DUBLICATE_ITEM_TABLE_SUCCESS',
);

export const EDIT_ITEM_TABLE_PENDING: InjectionToken<string> = new InjectionToken(
	'EDIT_ITEM_TABLE_PENDING',
);

export const EDIT_ITEM_TABLE_ERROR: InjectionToken<string> = new InjectionToken(
	'EDIT_ITEM_TABLE_ERROR',
);

export const EDIT_ITEM_TABLE_SUCCESS: InjectionToken<string> = new InjectionToken(
	'EDIT_ITEM_TABLE_SUCCESS',
);

export const CREATE_ITEM_TABLE_PENDING: InjectionToken<string> = new InjectionToken(
	'CREATE_ITEM_TABLE_PENDING',
);

export const CREATE_ITEM_TABLE_ERROR: InjectionToken<string> = new InjectionToken(
	'CREATE_ITEM_TABLE_ERROR',
);

export const CREATE_ITEM_TABLE_SUCCESS: InjectionToken<string> = new InjectionToken(
	'CREATE_ITEM_TABLE_SUCCESS',
);

export const GET_CURRENT_ITEM_PENDING: InjectionToken<string> = new InjectionToken(
	'GET_CURRENT_ITEM_PENDING',
);

export const GET_CURRENT_ITEM_ERROR: InjectionToken<string> = new InjectionToken(
	'GET_CURRENT_ITEM_ERROR',
);

export const GET_CURRENT_ITEM_SUCCESS: InjectionToken<string> = new InjectionToken(
	'GET_CURRENT_ITEM_SUCCESS',
);

export const SAVE_GRID_SETTINGS_PENDING: InjectionToken<string> = new InjectionToken(
	'SAVE_GRID_SETTINGS_PENDING',
);

export const SAVE_GRID_SETTINGS_ERROR: InjectionToken<string> = new InjectionToken(
	'SAVE_GRID_SETTINGS_ERROR',
);

export const SAVE_GRID_SETTINGS_SUCCESS: InjectionToken<string> = new InjectionToken(
	'SAVE_GRID_SETTINGS_SUCCESS',
);
export const SAVE_GRID_CHANGES_PENDING: InjectionToken<string> = new InjectionToken(
	'SAVE_GRID_CHANGES_PENDING',
);

export const SAVE_GRID_CHANGES_ERROR: InjectionToken<string> = new InjectionToken(
	'SAVE_GRID_CHANGES_ERROR',
);

export const SAVE_GRID_CHANGES_SUCCESS: InjectionToken<string> = new InjectionToken(
	'SAVE_GRID_CHANGES_SUCCESS',
);

export const GET_GRID_SETTINGS_PENDING: InjectionToken<string> = new InjectionToken(
	'GET_GRID_SETTINGS_PENDING',
);

export const GET_GRID_SETTINGS_ERROR: InjectionToken<string> = new InjectionToken(
	'GET_GRID_SETTINGS_ERROR',
);

export const GET_GRID_SETTINGS_SUCCESS: InjectionToken<string> = new InjectionToken(
	'GET_GRID_SETTINGS_SUCCESS',
);

export const MAKE_DEFAULT_GRID_PENDING: InjectionToken<string> = new InjectionToken(
	'MAKE_DEFAULT_GRID_PENDING',
);

export const MAKE_DEFAULT_GRID_ERROR: InjectionToken<string> = new InjectionToken(
	'MAKE_DEFAULT_GRID_ERROR',
);

export const MAKE_DEFAULT_GRID_SUCCESS: InjectionToken<string> = new InjectionToken(
	'MAKE_DEFAULT_GRID_SUCCESS',
);

export const RENAME_GRID_PENDING: InjectionToken<string> = new InjectionToken(
	'RENAME_GRID_PENDING',
);

export const RENAME_GRID_ERROR: InjectionToken<string> = new InjectionToken('RENAME_GRID_ERROR');

export const RENAME_GRID_SUCCESS: InjectionToken<string> = new InjectionToken(
	'RENAME_GRID_SUCCESS',
);

export const UPDATE_TABLE_STATE: InjectionToken<string> = new InjectionToken('UPDATE_TABLE_STATE');

export const CLEAR_CURRENT_ITEM: InjectionToken<string> = new InjectionToken('UPDATE_TABLE_STATE');
