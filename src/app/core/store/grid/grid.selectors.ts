...
import { GridInfo, IGridState } from './grid.state';

const gridState = (state: IAppState) => state.gridState;

export const selectGridData = createSelector(gridState, (state: IGridState, data) => {
	return state.gridData[data.gridId] || null;
});

export const selectGridUpdate = createSelector(gridState, (state: IGridState, data) => {
	return state.gridUpdate[data.gridId];
});

export const selectGridInfo = createSelector(gridState, (state: IGridState, data) => {
	return {
...
export const selectGridFilter = createSelector(gridState, (state: IGridState, data) => {
	const filters = state.gridFilters[data.gridId] || {};
	const result = filters[data.name] || null;
	return result;
});

export const selectCheckedAll = createSelector(gridState, (state: IGridState, data) => {
	return state.gridCheckAll[data.gridId] || false;
});

export const selectCheckedRows = createSelector(gridState, (state: IGridState, data) => {
	return state.gridCheckedLines[data.gridId] || {};
});

export const selectCheckedItems = createSelector(gridState, (state: IGridState, data) => {
...
export const selectGridSorting = createSelector(gridState, (state: IGridState, data) => {
	const sorting = state.gridSorting[data.gridId] || {};
	const result = sorting[data.name] || null;
	return result;
});

export const selectGridSelectedItem = createSelector(gridState, (state: IGridState, data) => {
